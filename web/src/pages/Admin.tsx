...
export default function Admin() {
  ...
  const { data: selectedWorkflow } = useQuery(
    ...
    async () => {
      ...
      const response = await axios.get(`/api/admin/workflows/${selectedWorkflowId}`)
  ...
  if (!adminKey || sessionLoading || (!isAuthorized && sessionError)) {
    return (
      <AdminUnlockPanel
        ...
        message={message}
        sessionErrorText={sessionError ? getErrorMessage(sessionErrorValue) : null}
  ...
  return (
    <div className="space-y-6 px-4 py-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Scanner Control Room</h2>
          <p className="mt-2 max-w-4xl text-sm text-gray-500 dark:text-gray-400">
            Workflow-first control room for continuous ACO scanning, one-off discovery runs, and the downstream jobs they create.
          </p>
        </div>

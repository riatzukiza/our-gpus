...
class IngestService:
    ...
    def parse_stream(self, data: bytes, mapping: Dict[str, str]) -> Generator[Dict[str, Any], None, None]:
    ...
    def process_batch(self, records: list, scan_id: int) -> Tuple[int, int]:
        ...
        for record in records:
            ...
            existing = self.session.query(Host).filter(
                Host.ip == record["ip"],
                Host.port == record.get("port", 11434)
            ).first()

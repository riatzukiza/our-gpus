...
from app.db import Host, Model, HostModel, Probe
...
class ProbeService:
    ...
    async def probe_host(self, host: Host) -> Probe:
        ...
        for attempt in range(self.retries):
            try:
                async with httpx.AsyncClient(verify=False, timeout=httpx.Timeout(self.timeout, connect=self.timeout)) as client:
                    ...
                    if tags_resp.status_code == 200:
                        ...
                        version_data = version_resp.json() if version_resp.status_code == 200 else {}
    ...
    def _extract_family(self, model_name: str) -> str:
        ...
        families = ["llama", "mistral", "mixtral", "phi", "gemma", "qwen", "deepseek", "codellama", "vicuna"]
    ...
    def _extract_parameters(self, model_data: Dict[str, Any]) -> Optional[str]:
    ...
    async def run_prompt(self, host_ip: str, host_port: int, model: str, prompt: str, stream: bool = False) -> Dict[str, Any]:
    ...
    async def stream_prompt(self, host_ip: str, host_port: int, model: str, prompt: str) -> AsyncGenerator[Dict[str, Any], None]:
        """Stream a prompt response from an Ollama instance"""
        base_url = f"http://{host_ip}:{host_port}"
        
        try:
            async with httpx.AsyncClient(verify=False, timeout=httpx.Timeout(60.0, connect=10.0)) as client:
                payload = {
                    "model": model,
                    "prompt": prompt,
                    "stream": True
                        ...
                        async for line in response.aiter_lines():
                            if line:
                                try:
                                    ...
                                    yield {"type": "error", "content": f"JSON decode error: {str(e)}"}
                                    continue
                    else:
                        error_text = await response.aread()
                        yield {"type": "error", "content": f"HTTP {response.status_code}: {error_text.decode('utf-8', errors='ignore')}"}
                        
        except httpx.TimeoutException:
            yield {"type": "error", "content": "Request timeout - the model may be loading or the prompt is too complex"}
        except httpx.ConnectError:
            yield {"type": "error", "content": f"Could not connect to host at {host_ip}:{host_port}"}
        except Exception as e:

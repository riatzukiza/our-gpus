...
class ACOMasscanScheduler:
    ...
    def get_stats(self) -> dict:
        ...
        with self._state_lock:
            return self.aco.stats()

    def get_top_blocks(self, n: int = 20) -> list[dict]:
        """Get top N blocks by pheromone score."""
        results = []
        with self._state_lock:
            for key, pheromone in self.aco.top_blocks(n):
                block = self.aco.blocks[key]
                results.append(
                    {
                        "cidr": key,
                        "pheromone": round(pheromone, 4),
                        "scan_count": block.scan_count,
                        "cumulative_yield": block.cumulative_yield,
                        "last_scan": block.last_scan_at.isoformat() if block.last_scan_at else None,
                    }
                )
        return results
